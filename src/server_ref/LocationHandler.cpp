...
LocationHandler::LocationHandler(ServerBlock *sb, const std::string &path)
{
	_sb = sb;
...
void	LocationHandler::init()
{
	...
	for (; i < _locationVec.size(); i++)
	{
		...
		if (_locationVec[i]->getPrefix().compare(_path) == 0)
		{
	...
	if (_location == NULL) // Porque estava um menos 1?
	{
